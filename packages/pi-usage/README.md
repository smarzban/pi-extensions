...
Run `/usage` to scan local session metadata and open today's centered overlay dashboard. The default view shows providers used through Pi only. Press `o` to show the other local sources, then Enter to drill from source to provider to model. Use `/usage today`, `/usage 7d`, `/usage 30d`, `/usage all`, or `/usage rebuild` for a full local index rebuild followed by today's view. Arrow keys select rows, while Left Arrow or Backspace returns to the previous level. Terminals narrower than 100 columns automatically use a compact inspector layout.
