...
test("a file removed between walk and read is removed from the current result", async () => {
 const root=await fixture(); try { const pi=join(root,"pi"), file=join(pi,"one.jsonl"); await jsonl(file,[line({type:"session",id:"one"}),line({type:"message",id:"one",timestamp:iso,message:{role:"assistant",usage:piUsage}})]); const initial=await importAll({pi}); assert.equal(initial.events.length,1); await writeFile(file,`${line({type:"message",id:"two",timestamp:iso,message:{role:"assistant",usage:piUsage}})}\n`,{flag:"a"}); const result=await importAll({pi},initial.index,{beforeRead:async path=>rm(path,{force:true})}); assert.equal(result.events.length,0); assert.equal(result.health[0].status,"partial"); assert.equal(result.health[0].reconciled,true); } finally { await rm(root,{recursive:true,force:true}); }
});
